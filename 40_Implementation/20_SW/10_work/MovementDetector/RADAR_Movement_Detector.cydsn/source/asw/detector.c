...
RC_t DETECTOR_init(Detector_t* detector){
    ...
    detector -> transferToUARTFinished = FALSE;
...
RC_t DETECTOR_startSampling(Detector_t* detector){
...
boolean_t DETECTOR_checkForSamplingCondition(Detector_t* detector){
    if (detector -> detectorState == IDLE){
        DETECTOR_startSampling(detector);
        return TRUE;
    }
    return FALSE;
...
RC_t DETECTOR_samplingFinished(Detector_t* detector){
    WaveDAC8_Stop();
    ADC_DelSig_StopConvert();
    detector-> samplingFinished = TRUE;
...
RC_t DETECTOR_startTransfer(Detector_t* detector){
    detector-> detectorState = UART_TRANSFER;
    detector -> samplingFinished = FALSE;
    CyDmaChEnable(DMA_MEM_UART_Chan, 1);
}

RC_t DETECTOR_transferToUARTFinished(Detector_t* detector){
    detector -> transferToUARTFinished = TRUE;
    CyDmaChDisable(DMA_MEM_UART_Chan);
}

RC_t DETECTOR_endTransfer(Detector_t* detector){

    detector -> numberOfTransfers = detector -> numberOfTransfers + 1;
...
RC_t DETECTOR_restartOperation(Detector_t* detector){
    DETECTOR_init(detector);
}



boolean_t DETECTOR_checkTransferInVariable(Detector_t* detector){
   uint8_t data = UART_LOG_GetByte();
    if ((detector ->detectorState == SAMPLING) && (data == IN_VARIABLE) && detector -> samplingFinished == TRUE){
        DETECTOR_startSampling(detector);
        return TRUE;
    }
    return FALSE;
...
boolean_t DETECTOR_checkTransferOutVariable(Detector_t* detector){
   uint8_t data = UART_LOG_GetByte();
    if ((detector ->detectorState == UART_TRANSFER) && (data == OUT_VARIABLE) && (detector -> transferToUARTFinished == TRUE)){
