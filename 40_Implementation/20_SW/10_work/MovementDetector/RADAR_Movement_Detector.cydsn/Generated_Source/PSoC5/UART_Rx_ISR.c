...
void UART_Rx_ISR_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)UART_Rx_ISR__INTC_NUMBER] = address;
}
...
cyisraddress UART_Rx_ISR_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)UART_Rx_ISR__INTC_NUMBER];
}
...
{
    *UART_Rx_ISR_INTC_PRIOR = priority << 5;
}
...
uint8 UART_Rx_ISR_GetPriority(void)
{
    ...
    priority = *UART_Rx_ISR_INTC_PRIOR >> 5;
